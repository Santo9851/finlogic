...
# Target OS: Ubuntu 22.04 LTS (ARM64) - e.g., Raspberry Pi 4 (8GB), AWS Graviton2, etc.
#
# ==============================================================================
#
# HOW TO USE:
# 1. Upload this script to your fresh Ubuntu server (e.g., via scp or copy-paste).
# 2. Make it executable: chmod +x deploy_arm.sh
# 3. Run as root: sudo ./deploy_arm.sh
#
# This script will:
...
install_packages() {
    ...
    log_info "Note: Docker officially supports ARM64 architecture."
    ...
    # ARM-specific Docker optimizations
    log_info "Applying ARM-specific Docker optimizations..."
    if [ ! -d "/etc/docker" ]; then
        mkdir -p /etc/docker
    fi
    
    # Create daemon.json for ARM optimizations if it doesn't exist
...
EOF
        log_success "Created Docker daemon configuration with ARM optimizations."
    else
...
services:
  backend:
    ...
    healthcheck:
      ...
      retries: 3
      start_period: 20s
    # ARM optimization: Limit memory usage for backend service
    deploy:
      resources:
        limits:
          memory: 512M
  ...
  redis:
    ...
    networks:
      - finlogic_net
    # ARM optimization: Limit memory usage for Redis
    deploy:
      resources:
        limits:
          memory: 256M
  ...
  frontend:
    ...
    networks:
      - finlogic_net
    # ARM optimization: Limit memory usage for frontend service
    deploy:
      resources:
        limits:
          memory: 512M
  ...
  celery_worker:
    ...
    networks:
      - finlogic_net
    # ARM optimization: Reduce concurrency for Celery worker on ARM
    deploy:
      resources:
        limits:
          memory: 256M
...
EXPOSE 8000
# ARM-optimized Gunicorn workers formula: (2 x CPU cores) + 1
# Using 4 workers as a safe default for most ARM servers
...
# ARM optimization: Increase memory limit for Node.js build if needed
# Note: Most ARM servers have sufficient memory for Next.js builds
RUN npm run build
...
EXPOSE 3000
# Use ARM-optimized Node.js if available (node:22-alpine already supports ARM)
CMD ["node", "server.js"]
EOF
...
echo -e ""
echo -e "${YELLOW}ARM-Specific Optimizations Applied:${NC}"
echo -e "• Docker daemon configuration with buildkit enabled"
echo -e "• Resource limits set for all services (backend, frontend, redis, celery)"
echo -e "• Reduced Celery worker concurrency for ARM efficiency"
echo -e ""

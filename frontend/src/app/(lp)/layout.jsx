...
/**
 * (lp)/layout.jsx – LP Portal sidebar layout
 */
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { LayoutDashboard, PieChart, FileText, LogOut, User } from 'lucide-react';
      ...
      <div className="flex h-screen bg-[#060010] overflow-hidden">
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-2.5 mx-2 rounded-lg mb-1 text-sm font-medium transition-all ${
                    active ? 'bg-[#0B6EC3]/15 text-[#0B6EC3]' : 'text-white/50 hover:text-white hover:bg-white/5'
                  }`}
                >
                  <Icon size={17} />
                  {label}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-14 flex items-center px-6 border-b border-white/8 bg-[#08001a]/80 backdrop-blur flex-shrink-0">
            <span className="ml-auto text-xs text-[#0B6EC3] bg-[#0B6EC3]/10 px-2.5 py-1 rounded-full font-medium">
              Limited Partner
            </span>
          </header>

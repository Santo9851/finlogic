...
import IndividualCapitalCallModal from '@/components/admin/IndividualCapitalCallModal';
...
export default function SuperAdminUserDetailPage({ params: paramsPromise }) {
  ...
  const [showCapitalCallModal, setShowCapitalCallModal] = useState(false);
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-12">
              ...
              {user.quota_history.length > 0 ? (
                user.quota_history.map((log, i) => (
                  <div key={log.id} className="p-6 bg-foreground/[0.01] border border-border-theme rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6 hover:border-purple-500/20 transition-all">
                    <div className="flex items-center gap-6">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-inner ${log.change_amount > 0 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-ls-secondary/10 text-ls-secondary'
                        }`}>
          ...
          {/* Institutional LP Section (Only for LPs/Investors) */}
          {(user.role_list.includes('lp') || user.role_list.includes('investor')) && (
            <div className="bg-card border border-border-theme rounded-[3rem] p-10 lg:p-12 shadow-2xl space-y-10 relative overflow-hidden theme-transition">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                <div>
                  <h3 className="text-xl font-black uppercase tracking-tight text-foreground">Institutional LP Portfolio</h3>
                  <p className="text-[10px] text-text-muted font-black uppercase tracking-[0.2em] opacity-40 mt-1">Capital Ledger & Equalization Management</p>
                </div>
                <button
                  onClick={() => setShowCapitalCallModal(true)}
                  className="px-8 py-4 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 transition-all shadow-xl shadow-purple-500/20 active:scale-95"
                >
                  <CircleDollarSign size={18} />
                  Issue Manual Capital Call
                </button>
              </div>

              <div className="p-8 bg-purple-500/5 border border-purple-500/10 rounded-3xl flex items-start gap-6 shadow-inner relative z-10">
                <div className="w-12 h-12 rounded-2xl bg-purple-500/10 flex items-center justify-center text-purple-500 shrink-0">
                  <Info size={24} />
                </div>
                <div className="space-y-2">
                  <p className="text-sm font-bold text-foreground">Equalization Protocol Notice</p>
                  <p className="text-[11px] text-text-muted leading-relaxed">
                    Use this interface to issue capital calls for late-joining LPs.
                    The system automatically identifies "Equalization Gaps" (projects funded before this LP joined)
                    and suggests the exact NPR amount required to bring this LP to pro-rata parity with the existing pool.
                  </p>
                </div>
              </div>
            </div>
          ...
          {showCapitalCallModal && (
            <IndividualCapitalCallModal
              lpProfileId={user.lp_profile_id}
              onClose={() => setShowCapitalCallModal(false)}
              onRefresh={() => queryClient.invalidateQueries(['superadmin', 'user', id])}
            />
          )}
        </div>
      </div>

    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="bg-[#140b2e] border border-white/10 rounded-[2.5rem] p-8 relative overflow-hidden shadow-2xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6 border-b border-white/5 pb-8">
          <div>
            <h3 className="text-3xl font-black text-white tracking-tight uppercase tracking-tighter">Operational Audit</h3>
            <p className="text-white/40 text-sm mt-1 uppercase tracking-widest font-bold">Tech Stack, Team Risk & Supply Chain Review</p>
          </div>
          
          <div className="flex items-center gap-3">
            <button 
              onClick={() => window.location.reload()}
              className="flex items-center justify-center gap-2 px-4 py-4 bg-white/5 text-white/40 hover:text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border border-white/10"
        ...
        {isProcessing && (
          <div className="mb-8 p-6 bg-[#F59F01]/10 border border-[#F59F01]/20 rounded-2xl flex items-center justify-between animate-pulse">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-white font-black text-sm uppercase tracking-widest">AI Audit in Progress</p>
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-full border border-white/10">
              ...
              <span className="text-[10px] font-black text-white/40 uppercase tracking-tighter">Polling Status...</span>
            </div>
          </div>
        )}
        ...
        {/* Preparation Overlay */}
        {showPrep && (
          <div className="fixed inset-0 z-[100] bg-[#100226]/80 backdrop-blur-2xl flex items-center justify-center p-6 md:p-10 animate-in fade-in zoom-in-95 duration-300">
            <div className="max-w-2xl w-full max-h-[90vh] overflow-y-auto space-y-8 bg-[#140b2e] border border-white/10 p-8 md:p-12 rounded-[3rem] shadow-2xl relative">
               <button 
               ...
               <div className="text-center space-y-4">
                  <h3 className="text-3xl font-black text-white uppercase tracking-tighter">Strategic Preparation</h3>
               ...
               <div className="space-y-6">
                  <label className="flex items-start gap-4 p-6 bg-white/[0.03] border border-white/5 rounded-2xl cursor-pointer hover:bg-white/[0.05] transition-all group">
                    <input 
                      type="checkbox" 
                      ...
                      className="mt-1 w-5 h-5 rounded border-white/10 bg-white/5 text-[#F59F01] focus:ring-[#F59F01]"
                    />
                    <div>
                      <p className="text-white font-bold text-sm uppercase tracking-wider">Data Room Readiness</p>
                      <p className="text-white/30 text-xs mt-1">I have uploaded Tech Stack, Org Charts, and Operational Audit documents to the Data Room.</p>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-4 p-6 bg-white/[0.03] border border-white/5 rounded-2xl cursor-pointer hover:bg-white/[0.05] transition-all group">
                    ...
                    />
                    <div>
                      <p className="text-white font-bold text-sm uppercase tracking-wider">Manual Operational Context</p>
                      <p className="text-white/30 text-xs mt-1">I will provide expert context about the business operations to ground the analysis.</p>
                    </div>
                  </label>
                  ...
                  {provideManual && (
                    <div className="space-y-4 animate-in slide-in-from-top-4 duration-300">
                      <textarea 
                        ...
                        className="w-full h-48 bg-black/40 border border-[#F59F01]/20 rounded-2xl p-6 text-white text-sm focus:border-[#F59F01] focus:ring-1 focus:ring-[#F59F01] transition-all placeholder:text-white/10"
                      />
               ...
               <div className="flex items-center gap-4 pt-4">
                  <button 
                    onClick={() => setShowPrep(false)}
                    className="flex-1 px-8 py-5 bg-white/5 text-white/50 rounded-2xl text-xs font-black uppercase tracking-widest hover:text-white transition-all"
                  >
                    Cancel
                  </button>
                  <button 
                    onClick={handleRun}
                    disabled={!canRun}
                    className="flex-[2] flex items-center justify-center gap-2 px-8 py-5 bg-[#F59F01] text-black rounded-2xl text-xs font-black uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-[#F59F01]/20 disabled:opacity-20 disabled:grayscale"
                  >
        ...
        {analysis && !isProcessing ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-8 space-y-8">
               <div className={`bg-black/30 rounded-[2rem] border transition-all duration-500 relative group overflow-hidden ${isEditing ? 'border-[#F59F01]/30 ring-1 ring-[#F59F01]/10' : 'border-white/5'}`}>
                  <div className="absolute top-8 right-10 flex items-center gap-2 opacity-20 group-hover:opacity-100 transition-opacity z-10">
                    <ShieldCheck size={14} className={isEditing ? 'text-[#F59F01]' : 'text-emerald-400'} />
            ...
            <div className="lg:col-span-4 space-y-6">
               {/* Tech Stack Card */}
               <div className="bg-white/[0.03] border border-white/10 p-6 rounded-3xl relative overflow-hidden">
                  <div className="flex items-center gap-2 mb-6">
                    <Cpu size={18} className="text-[#F59F01]" />
                    <h4 className="text-[10px] font-black text-white uppercase tracking-widest tracking-tighter">Core Technology Stack</h4>
                  </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                           {Object.entries(analysis.technology_stack).map(([key, val]) => (
                              <div key={key} className="bg-white/5 p-3 rounded-xl border border-white/5">
                                 <p className="text-[8px] text-white/30 uppercase font-black mb-1">{key}</p>
                                 <p className="text-white text-[10px] font-bold truncate">{String(val)}</p>
                              </div>
                           ))}
                        </div>
                     ) : (
                  ...
                  {(!analysis.technology_stack || Object.keys(analysis.technology_stack).length === 0) && (
                    <p className="text-white/20 text-xs italic">Awaiting technical audit completion.</p>
                  )}
               </div>
               ...
               <div className="bg-black/30 border border-white/5 p-6 rounded-3xl">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-[10px] font-black text-white uppercase tracking-widest tracking-tighter">Founder Dependency</h4>
                    </div>
                  ...
                  <div className="flex items-end justify-between">
                    <div className="text-4xl font-black text-white tabular-nums">{analysis.key_person_risk_score}<span className="text-lg text-white/20">/10</span></div>
                    <p className="text-[10px] text-white/40 uppercase font-bold mb-1">Risk Score</p>
                  </div>
                  ...
                  <div className="space-y-4">
                    {(analysis.supply_chain_risks || []).map((risk, i) => (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <span className="text-[10px] text-white/80 leading-relaxed">{risk}</span>
                      </div>
                    ))}
                    {(!analysis.supply_chain_risks || analysis.supply_chain_risks.length === 0) && (
                      <p className="text-white/20 text-xs italic">No vulnerabilities identified.</p>
                    )}
                  </div>
               </div>
               ...
               <div className="bg-red-500/5 border border-red-500/10 p-6 rounded-3xl">
                  ...
                  <div className="space-y-3">
                    {(analysis.operational_red_flags || [])
                      ...
                      .map((flag, i) => (
                      <div key={i} className="p-3 bg-black/40 rounded-xl border border-red-500/10 text-[10px] text-white/70 leading-relaxed font-serif italic">
                        {flag}
                      </div>
                    ))}
                    {(!analysis.operational_red_flags || analysis.operational_red_flags.filter(f => f.length <= 200).length === 0) && (
                      <p className="text-white/20 text-xs italic text-center">Clean audit. No critical blockers.</p>
                    )}
                  </div>
               </div>
            </div>
          ...
          <div className="py-32 text-center space-y-6">
             <div className="w-24 h-24 rounded-[2rem] bg-white/5 flex items-center justify-center text-white/10 mx-auto border border-white/5">
                {isProcessing ? (
             ...
             <div className="max-w-xs mx-auto">
                ...
                <p className="text-white/20 text-sm mt-2 tracking-wide font-medium leading-relaxed">
                  {isProcessing 
